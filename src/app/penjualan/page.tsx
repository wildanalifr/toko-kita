import { fetchPenjualan } from '@/api/penjualan'
import { formatCurrency } from '@/utils/formatCurrency'
import { fDateTime } from '@/utils/formatTime'
import Link from 'next/link'
import { LineChart, PlusCircle, AlertCircle } from 'lucide-react'

export default async function Penjualan() {
  const { data, error } = await fetchPenjualan()
    ...
    <div className="bg-gray-900 rounded-xl p-6 shadow-md mt-16">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2 text-white">
          ...
          <p className="text-xl font-semibold">Riwayat Penjualan</p>
        </div>
        <Link
          href="/penjualan/new"
          ...
          {data?.length ? (
            data.map((item) => (
              ...
              >
                ...
                <td className="py-2 px-4">{formatCurrency(item.harga_jual)}</td>
                <td className="py-2 px-4">
                  {formatCurrency(item.hpp as number)}
                </td>
              </tr>
            ))
          ) : (
